...
import { applyAverageSupport, pairwiseResults } from "./pipeline/pairwise";
...
export const copelandWeighted = (
  ...
  // Process votes through our pipeline
  const numberOfChoices = orderedChoices.length;

  // Get pairwise comparisons and then process the results
  const { pairwiseResults: pairwiseComparison, matchStats } = pairwiseResults(
    cleanedVotes,
    numberOfChoices,
  );

  // Apply scoring pipeline
  const finalResults = pipe(pairwiseComparison)
    .through((pairwise) => applyAverageSupport(pairwise, matchStats))
    .through((withSupport) => calculatePoints(withSupport, [1, 0, 0]))
