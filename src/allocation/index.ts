...
export const allocateBudgets = (
  ...
  const scoringResults = displayResults(
    results, 
    orderedChoices, 
    { 
      algorithm: "copeland-none-below", 
  ...
  const rankedCandidates: RankedCandidate[] = scoringResults.map(result => {
    ...
    const choiceIndex = orderedChoices.findIndex(c => c.choice === result.choice);
    ...
    if (candidateGroup) {
      ...
      if (sameGroupIndices.length > 0) {
        ...
        const winsInternalMatchups = sameGroupIndices.every(opponentIndex => {
          ...
          const matchupResult = results.find(r => Number(r.key) === choiceIndex);
  ...
  for (const candidate of rankedCandidates) {
    ...
    if (options.noneBelowOption) {
      const noneBelowIndex = orderedChoices.findIndex(c => 
        c.choice === options.noneBelowOption);
      
      if (noneBelowIndex !== -1) {
        const candidateIndex = orderedChoices.findIndex(c => 
          c.choice === choice);
  ...
  if (spp1Allocated >= MAX_SPP1_ALLOCATIONS || 
    ...
    if (totalOneYear > 0) {
      ...
      remainingBudget.oneYear.basic += 
        Math.floor(remainingTwoYearTotal * basicProportion);
      remainingBudget.oneYear.extended += 
        Math.floor(remainingTwoYearTotal * (1 - basicProportion));
